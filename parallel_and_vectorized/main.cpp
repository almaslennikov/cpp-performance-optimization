    ...
    for (int i = 0; i < 10; i++)
    {
        for (auto filter : filters)
        {
            group.run([filter, &registry]
            {
                ...
                IFilter<std::array<char, 32>>::Ptr nameFilter = nullptr;
                auto nameFilterIt = filter.find("name");
                if (nameFilterIt != filter.end())
                {
                    nameFilter = std::holds_alternative<IFilter<std::array<char, 32>>::Ptr>(nameFilterIt->second) 
                        ? std::get<IFilter<std::array<char, 32>>::Ptr>(nameFilterIt->second)
                        : nullptr;
                }

                IFilter<std::array<char, 32>>::Ptr positionFilter = nullptr;
                auto positionFilterIt = filter.find("position");
                if (positionFilterIt != filter.end())
                {
                    positionFilter = std::holds_alternative<IFilter<std::array<char, 32>>::Ptr>(positionFilterIt->second)
                        ? std::get<IFilter<std::array<char, 32>>::Ptr>(positionFilterIt->second)
                        : nullptr;
                }
